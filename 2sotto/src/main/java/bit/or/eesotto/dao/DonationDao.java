...
public interface DonationDao {
	...
	//전체 게시물
	/*
	 * @Select("SELECT dindex, title, dobject, rtime FROM (SELECT ROWNUM NUM, N.* FROM (SELECT * FROM Donation WHERE ${param2} ORDER BY rtime DESC) N) WHERE RNUM BETWEEN 1+(${param1}-1)*5 AND 5+(${param1}-1)*5"
	 * )
	 */
	...
	/*
	 * @Select("SELECT E.*" +
	 * "FROM (SELECT @ROWNUM := @ROWNUM +1 ROWNUM, E.* FROM Donation E WHERE (@rownum:=0)=0) E"
	 * +
	...
	/*
	 * @Select("SELECT dindex, title, content, dobject, rtime, FROM Donation WHERE dindex=#{dindex}"
	 * )
	 */
